...
//		}
		...
		if (!sim_->agents_.empty()) {
			buildAgentTreeRecursive(0, sim_->agents_.size(), 0);
		}
	}
	...
	void KdTree::buildAgentTreeRecursive(size_t begin, size_t end, size_t node)
	{
		...
		agentTree_[node].minX = agentTree_[node].maxX = sim_->agents_[begin]->position_.x();
		agentTree_[node].minY = agentTree_[node].maxY = sim_->agents_[begin]->position_.y();

		for (size_t i = begin + 1; i < end; ++i) {
			agentTree_[node].maxX = std::max(agentTree_[node].maxX, sim_->agents_[i]->position_.x());
			agentTree_[node].minX = std::min(agentTree_[node].minX, sim_->agents_[i]->position_.x());
			agentTree_[node].maxY = std::max(agentTree_[node].maxY, sim_->agents_[i]->position_.y());
			agentTree_[node].minY = std::min(agentTree_[node].minY, sim_->agents_[i]->position_.y());
		}
		...
		if (end - begin > MAX_LEAF_SIZE) {
			...
			while (left < right) {
				while (left < right && (isVertical ? sim_->agents_[left]->position_.x() : sim_->agents_[left]->position_.y()) < splitValue) {
					++left;
				}

				while (right > left && (isVertical ? sim_->agents_[right - 1]->position_.x() : sim_->agents_[right - 1]->position_.y()) >= splitValue) {
					--right;
				}

				if (left < right) {
					std::swap(sim_->agents_[left], sim_->agents_[right - 1]);
					++left;
					--right;
				}
			}
	...
	void KdTree::queryAgentTreeRecursive(Agent *agent, float &rangeSq, size_t node) const
	{
		if (agentTree_[node].end - agentTree_[node].begin <= MAX_LEAF_SIZE) {
			for (size_t i = agentTree_[node].begin; i < agentTree_[node].end; ++i) {
				agent->insertAgentNeighbor(sim_->agents_[i], rangeSq);
			}
		}
		else {
