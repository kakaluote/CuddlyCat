...
namespace RVO {
    ...
    size_t RVOSimulator::addObstacle(const std::vector<Vector2> &vertices) {
        ...
        obstaclesPts.push_back(vertices);
    ...
    void RVOSimulator::removeAllObstacle() {
        obstacles_.clear();
        obstaclesPts.clear();
...
#pragma omp parallel for
    ...
    void RVOSimulator::removeAllAgent() {
        agents_.clear();
    }

    void RVOSimulator::setAgentGoal(size_t agentNo, float x, float y) {
        agents_[agentNo]->setGoal(x, y);
    }

    void RVOSimulator::setAgentGoal(size_t agentNo, const Vector2 &p) {
        agents_[agentNo]->setGoal(p);
    }

    bool RVOSimulator::isAgentReachGoal(size_t agentNo, float small_dist) {
        return agents_[agentNo]->isReachGoal(small_dist);
    }

    Vector2 RVOSimulator::getAgentGoal(size_t agentNo) {
    ...
    void
    RVOSimulator::setAgentDefaults(float neighborDist, size_t maxNeighbors, float timeHorizon, float timeHorizonObst,
