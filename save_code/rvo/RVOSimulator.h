...
namespace RVO {
	...
	public:
		...
		const std::vector< std::vector<RVO::Vector2> >& getObstaclesPts(){
			return obstaclesPts;
		}
		void setAgentGoal(size_t agentNo,float x,float y);
		bool isAgentReachGoal(size_t agentNo,float small_dist);
		Vector2 getAgentGoal(size_t agentNo);
		void removeAgent(size_t agentNo);
		void removeAllAgent();
		void removeAllObstacle();
		void setAgentGoal(size_t agentNo, const Vector2& p);
	...
	private:
		...
		std::vector< std::vector<RVO::Vector2> > obstaclesPts;
		float timeStep_;
