...
namespace RVO {
	...
	class Agent {
	public:
		inline void setGoal(float x,float y){
			goal_.x_ = x;
			goal_.y_ = y;
		}
		inline void setGoal(const Vector2& p){
			goal_.x_ = p.x();
			goal_.y_ = p.y();
		}
		inline bool isReachGoal(float small_dist){
			return absSq(position_ - goal_) < small_dist*small_dist;
		}
